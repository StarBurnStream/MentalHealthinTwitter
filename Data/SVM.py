...
def main():
    get_results.csvToResultsV2("Data/newPlatformDataset.csv")
...
def dataImitation():
    ...
    dataimitation = open("Data/Imitation/imitation.csv","a")
    dataimitation.write("depressed or not,volume,retweets,reply,questions,links,numFollowers,numFollowees,depression,swear,I,we,they,shehe,positiveemo,negativeemo\n")
    ...
    print(depressedList,undepressedList)
    vectorList = []
    liwcDict = Utility.deserialize('Data/Imitation/liwcDict.json')
    k = 0
    for user in (depressedList + undepressedList):
        k += 1
        print( str(k) + 'th user being analyzed...')
        if user in depressedList:
            newUser, data = attributesWithoutLIWC.updateImitationEngagements(user,depressed=True)
            vector = [1]
        else:
            newUser, data = attributesWithoutLIWC.updateImitationEngagements(user,depressed=False)
            vector = [0]
        ...
        vector += [newUser.volume, newUser.retweets, newUser.reply, newUser.questions, newUser.links,
                      newUser.numFollowers, newUser.numFollowees] + liwcDict[str(user)[1:]]
        linestr = ''
        for item in vector:
            linestr += str(item) + ','
        linestr = linestr[:-1] + '\n'
        dataimitation.write(linestr)
    dataimitation.close()
    return
...
def trainSVM():
    ...
    vectorList = []
    targetList = []
    liwcDict = Utility.deserialize('Data/Training/liwcDict.json')
    ...
    for userid in idList:
        ...
        if userid not in userDict:
            ...
            act /= len(data)
            vector = [newUser.volume, newUser.retweets, newUser.reply, newUser.questions, newUser.links,
                      newUser.numFollowers, newUser.numFollowees] + [pa,na,dom,act]#liwcDict[str(userid)]
            userDict[userid] = vector
        else:
            vector = userDict[userid]
        vectorList.append(vector)
        targetList.append(scoreList[idList.index(userid)])
        targetList = ['24','12','0','0','16','0']  # Trial
        #targetList = ['1','1','0','0','1','0']  # Trial
    print(vectorList)
    print(targetList)
...
def predictUserList(clf):
    ...
    userList = ['14835269','898264108914667520','225761199'] # Trial,'532688309'
    liwcDict = Utility.deserialize('Data/Predicting/liwcDict.json')
    ...
    for userid in userList:
        ...
        if userid not in predictionDict:
            ...
            vector = [user.volume, user.retweets, user.reply, user.questions, user.links,
                      user.numFollowers, user.numFollowees] + [pa, na, dom, act] #liwcDict[str(userid)]  
...
def testSVM():
    filename = "Data/testTrain.csv"
    trainList = Utility.readCsv(filename)[1:]
    vectorList = []
    targetList = []
    zero = trainList[0][-1]
    trainList = trainList[:-1]
    for row in trainList:
        vectorList.append(row[:-1])
        if row[-1] == zero:
            targetList.append(0)
        else:
            targetList.append(1)
    ...
    filename = "Data/testPredict.csv"
    trainList = Utility.readCsv(filename)[1:]
    resultList = []
    trainList = trainList[:-1]
    for row in trainList:
        vectorList.append(row[:-1])
        resultList.append(clf.predict([row[:-1]]))
    print(resultList)
'''
