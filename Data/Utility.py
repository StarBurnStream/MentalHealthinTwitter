...
def checkUserListNameID():
    ...
    for worker in workerInfo[1:-1]:
        ...
        if worker[3] != "None" and worker[3] != "none" and (aname[0] or aid[0]):
...
def getNewYorkTime(dateStr):
    targetTimeZone = pytz.timezone('America/New_York')
    date = dateStrToDate(dateStr)
    localTimeZone = pytz.timezone('utc')
    date = date.replace(tzinfo=localTimeZone)
    date = date.astimezone(targetTimeZone)
...
#print(int("0"))
'''
a=dateStrToDate("Tue Feb 20 00:00:00 +0000 2018")
b=dateStrToDate("Tue Feb 20 00:00:00 +0000 2018")
local = pytz.timezone('utc')
a=a.replace(tzinfo=local)
b=b.replace(tzinfo=local)
print(a)
use = pytz.timezone('America/New_York')
a=a.astimezone(use)
print(a>b,a==b,a<b)
print(dateToDateStr(a))
'''
